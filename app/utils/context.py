...
def create_interpretation_prompt(user_query: str, tool_results: List[Dict[str, Any]], report_type: str = None, context_insights: List[str] = None) -> str:
    ...
    prompt_parts.append("")
    prompt_parts.append("Example 1 - Success Rate Query:")
    prompt_parts.append("├── User Query: 'What is the success rate of customer.csv?'")
    prompt_parts.append("├── Tool Results: {'success_rate': 87.5, 'total_records': 1200, 'file_name': 'customer.csv'}")
    prompt_parts.append("└── Expected Response: 'The customer.csv file shows a success rate of 87.5% across 1,200 records. This indicates good overall performance with room for improvement in the remaining 12.5% of cases.'")
    prompt_parts.append("")
    prompt_parts.append("Example 2 - Failure Analysis:")
    prompt_parts.append("├── User Query: 'Show me failure rate for product domain'")
    prompt_parts.append("├── Tool Results: {'failure_rate': 15.2, 'failed_records': 152, 'domain_name': 'product'}")
    prompt_parts.append("└── Expected Response: 'The product domain has a failure rate of 15.2%, affecting 152 records. This moderate failure rate suggests specific validation or processing issues that need attention.'")
    prompt_parts.append("")
    prompt_parts.append("Example 3 - Distribution Analysis:")
    prompt_parts.append("├── User Query: 'How many customers by country?'")
    prompt_parts.append("├── Tool Results: [{'country': 'USA', 'count': 450}, {'country': 'UK', 'count': 320}, {'country': 'Canada', 'count': 180}]")
    prompt_parts.append("└── Expected Response: 'Customer distribution shows USA leading with 450 customers (47%), followed by UK with 320 (34%) and Canada with 180 (19%). The concentration in these three markets represents strong regional presence.'")
    prompt_parts.append("")
    prompt_parts.append("Example 4 - Trend Analysis:")
    prompt_parts.append("├── User Query: 'Show me success rate from last week'")
    prompt_parts.append("├── Tool Results: {'avg_success_rate': 92.3, 'trend': 'improving', 'previous_week': 88.1}")
    prompt_parts.append("└── Expected Response: 'Success rates improved to 92.3% this week, up from 88.1% last week—a positive 4.2% increase. This upward trend indicates recent optimizations are working effectively.'")
    prompt_parts.append("")
    prompt_parts.append("NOW, apply these patterns to the CURRENT user query and tool results above.")
